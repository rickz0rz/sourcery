...
func TestAcquireUntilExhausted(t *testing.T) {
	a := fleet(t, map[string]int{"flex": 2})

	first, ok := a.TryAcquire(cand("flex"))
	if !ok {
		t.Fatal("first acquire failed")
	}
	if _, ok := a.TryAcquire(cand("flex")); !ok {
...
func TestUnreachableDeviceGetsNoCapacity(t *testing.T) {
	a := New([]device.State{{
		Device: &device.Device{Device: config.Device{Name: "prime"}},
		Err:    errors.New("unreachable"),
	}})
	if _, ok := a.TryAcquire(cand("prime")); ok {
...
func TestReleaseIsIdempotent(t *testing.T) {
	a := fleet(t, map[string]int{"flex": 1})

	lease, _ := a.TryAcquire(cand("flex"))
	lease.Release()
	lease.Release()

	if held := status(t, a, "flex").Held; held != 0 {
...
func TestReconcileSeparatesForeignFromOwnUsage(t *testing.T) {
	a := fleet(t, map[string]int{"prime": 3})

	lease, _ := a.TryAcquire(cand("prime"))
	...
	lease.Release()
	if st := status(t, a, "prime"); st.Free != 2 {
...
func TestReconcileClampsAtZero(t *testing.T) {
	...
	l1, _ := a.TryAcquire(cand("prime"))
	l2, _ := a.TryAcquire(cand("prime"))
	defer l1.Release()
	defer l2.Release()

	a.Reconcile("prime", 1) // stale reading, taken before our second stream
...
func TestForeignUsageExhaustsCapacity(t *testing.T) {
	...
	if free := status(t, a, "prime").Free; free != 0 {
...
func TestConcurrentAcquireRespectsCapacity(t *testing.T) {
	const tuners = 4
	a := fleet(t, map[string]int{"flex": tuners})
	...
	for range 50 {
		...
		go func() {
			defer wg.Done()
			if lease, ok := a.TryAcquire(cand("flex")); ok {
				mu.Lock()
